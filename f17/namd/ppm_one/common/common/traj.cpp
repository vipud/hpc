...
#pragma acc enter data create(ani_effect_arr[0:select_size])
	...
	double *debug_inner = new double[index_size*select_size*8];

	for(i=0;i<nframe;i++)
	{
		base=i*natom;
#pragma acc parallel copy(ani_effect_flat[0:select_size*index_size*4]) copyout(debug_i[0:3*index_size], debug_v1[0:3*index_size], debug_v2[0:3*index_size], debug_ori[0:3*index_size], debug_center[0:3*index_size], debug_extra[0:3*index_size], debug_inner[0:index_size*select_size*8]) present(index[0:index_size], select[0:select_size], my_x_arr[0:my_x_size], my_y_arr[0:my_y_size], my_z_arr[0:my_z_size]) private(center[0:3],v1[0:3],v2[0:3],ori[0:3],i1,i2,i3,e,cosa,length,jj,k)
{
#pragma acc loop 
...
#pragma acc loop seq
			...
			{
				e=0;	
				debug_inner[j*select_size*8 + jj*8 + 0] = e;
#pragma acc loop seq							
				...
				debug_inner[j*select_size*8 + jj*8 + 1] = i1;
				debug_inner[j*select_size*8 + jj*8 + 2] = v1[0];
				debug_inner[j*select_size*8 + jj*8 + 3] = v1[1];
				debug_inner[j*select_size*8 + jj*8 + 4] = v1[2];
				debug_inner[j*select_size*8 + jj*8 + 5] = length;
				debug_inner[j*select_size*8 + jj*8 + 6] = cosa;
				debug_inner[j*select_size*8 + jj*8 + 7] = e;
