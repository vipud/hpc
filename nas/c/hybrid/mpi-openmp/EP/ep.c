...
#define FIRST(RANK,SIZE,MSIZE)  (RANK*SIZE/MSIZE)
...
int main(int argc, char *argv[])
{
  ...
  int first=FIRST(rank,2*NK,mpi_size);
  int numOwn=FIRST(rank+1,2*NK,mpi_size)-first;
  double xx[numOwn];
...
///////////////////////////////
  ...
  MPI_Gather(&xx,numOwn,MPI_DOUBLE, &x,numOwn,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
if (rank==0){
  ...
  k_offset = -1;
  int first1=FIRST(rank,NQ,mpi_size);
  int numOwn1=FIRST(rank+1,NQ,mpi_size)-first1;

  int first2=FIRST(rank,np,mpi_size);
  int numOwn2=FIRST(rank+1,np,mpi_size)-first1;
  ...
  MPI_Reduce(&q,&qg,NQ,MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
  

  for (i = first1; i < numOwn1; i++) {
    gc = gc + q[i];
  }
  MPI_Barrier(MPI_COMM_WORLD);
  double ggc;
  MPI_Reduce(&gc,&ggc,1,MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
  gc=ggc;
if (rank==0){
  timer_stop(0);
  tm = timer_read(0);
