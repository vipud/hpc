...
int main() 
{
  ...
  //iii : initialize the MPI environment
  MPI_Init(NULL, NULL);

  //iv : get total number of tasks
  MPI_Comm_size(MPI_COMM_WORLD, &numTasks);

  //v: get the task's rank
  MPI_Comm_rank(MPI_COMM_WORLD, &taskRank);

  //vi : get the name of the current processor
  MPI_Get_processor_name(hostName, &len);

  //vii : hello world!
  printf("Hello from Task: %d, Host: %s \n",taskRank,hostName);


  //viii : print from the master task
  if(taskRank == 0){
          printf("Hello from MASTER. there are %d tasks \n", numTasks);
  }
